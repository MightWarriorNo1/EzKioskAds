    ...
    try {
      switch (integration.type) {
        case 'gmail':
          ...
          const gmailAuthUrl = `https://accounts.google.com/oauth/authorize?client_id=${process.env.REACT_APP_GOOGLE_CLIENT_ID}&redirect_uri=${encodeURIComponent(window.location.origin + '/admin/integrations')}&scope=https://www.googleapis.com/auth/gmail.send&response_type=code`;
          window.open(gmailAuthUrl, '_blank');
          break;
        case 'google_drive':
          // Redirect to Google Drive OAuth
          const driveAuthUrl = `https://accounts.google.com/oauth/authorize?client_id=${process.env.REACT_APP_GOOGLE_CLIENT_ID}&redirect_uri=${encodeURIComponent(window.location.origin + '/admin/integrations')}&scope=https://www.googleapis.com/auth/drive&response_type=code`;
