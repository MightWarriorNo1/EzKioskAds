    ...
    <div className="space-y-6">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {revenueSummary.total_impressions.toLocaleString()}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">{getTimeRangeLabel()}</p>
            </div>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {revenueSummary.total_clicks.toLocaleString()}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">{getTimeRangeLabel()}</p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Cost Per Mille (CPM)</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          {getTopPerformingKiosks().map((kiosk, index) => (
            <div key={index} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <div className="flex items-center space-x-4">
                <div className="w-8 h-8 bg-green-100 dark:bg-green-900/20 rounded-full flex items-center justify-center text-green-600 dark:text-green-400 font-bold text-sm">
                  #{index + 1}
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{kiosk.name}</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {kiosk.impressions.toLocaleString()} impressions, {kiosk.clicks.toLocaleString()} clicks
                  </p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-sm font-medium text-gray-900 dark:text-white">
                  ${kiosk.revenue.toFixed(2)}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  CTR: {kiosk.impressions > 0 ? ((kiosk.clicks / kiosk.impressions) * 100).toFixed(2) : 0}%
                </p>
              </div>
            </div>
          ))}
